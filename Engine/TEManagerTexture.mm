...
static std::map<String, uint> mTextures;

uint TEManagerTexture::GLUtexImage2D(CGImageRef cgImage, String textureName) {
    uint texture;
    if (mTextures.count(textureName) > 0) {
        NSLog(@"TEManagerTexture::GLUtexImage2D cache hit");
        texture = mTextures[textureName];
    } else {
        NSLog(@"TEManagerTexture::GLUtexImage2D texture doesn't exist in cache");
        NSUInteger width;
        NSUInteger height;
        float i;
